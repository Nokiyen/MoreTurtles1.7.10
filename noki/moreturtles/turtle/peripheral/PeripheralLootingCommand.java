...
public class PeripheralLootingCommand implements ITurtleCommand {
	...
	//******************************//
	public PeripheralLootingCommand(TurtleSide side, int dir) {
	...
	public TurtleCommandResult execute(ITurtleAccess givenTurtle) {
		...
		turtle.playerTurtle.rotationYaw = yaw[this.dir];
		turtle.playerTurtle.rotationPitch = pitch[this.dir];
		
		Vec3 turtlePos = Vec3.createVectorHelper(turtle.playerTurtle.posX, turtle.playerTurtle.posY, turtle.playerTurtle.posZ);
	...
	private EntityLivingBase getNearestLivingEntity(World world, Vec3 vecStart, Vec3 vecDir, double distance) {
		...
		AxisAlignedBB rangeBox = AxisAlignedBB.getBoundingBox(
...
/*				if(entityBox.isVecInside(vecStart)) {
				...
				if(entityBox.isVecInside(vecStart)) {
					...
					Vec3 vecEntity = Vec3.createVectorHelper(entity.posX, entity.posY, entity.posZ);
